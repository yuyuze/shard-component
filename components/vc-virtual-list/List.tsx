...
import type { Key } from '../../util/type';
...
function renderChildren<T>(
  ...
  return list.slice(startIndex, endIndex + 1).map((item, index) => {
    ...
    return (
      <Item
        key={`${key}${eleIndex}`}
        setRef={(ele) => setNodeRef(item, ele as HTMLElement)}
      >
        {node}
      </Item>
    );
...
const List = defineComponent({
  ...
  props: {
    ...
    itemKey: {
      type: [String, Number, Function] as PropType<
        Key | ((item: Record<string, any>) => Key)
      >,
      required: true
    },
    component: {
  ...
  setup(props, { expose }) {
        ...
        setTimeout(() => {
          ...
          for (let i = 0; i < dataLen; i += 1) {
            ...
            const currentItemBottom =
              itemTop +
              (cacheHeight === undefined ? props.itemHeight! : cacheHeight);

            if (
              currentItemBottom >= state.scrollTop &&
            ...
            if (
              currentItemBottom > state.scrollTop + props.height! &&
              endIndex === undefined
            ) {
              endIndex = i;
            }
    ...
    const maxScrollHeight = computed(
      () => calRes.value.scrollHeight! - props.height!
    );
    ...
    const isScrollAtBottom = computed(
      () => state.scrollTop >= maxScrollHeight.value
    );
