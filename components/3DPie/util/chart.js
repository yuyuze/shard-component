...
const getPie3D = (
  ...
  for (let i = 0; i < series.length; i++) {
    endValue = Number(startValue) + Number(series[i].pieData.value);
    series[i].pieData.startRatio =
      sumValue !== 0 ? startValue / Number(sumValue) : 0;
    series[i].pieData.endRatio =
      sumValue !== 0 ? endValue / Number(sumValue) : 0;
    ...
    startValue = endValue;
    const bfb =
      sumValue !== 0 ? fomatFloat(series[i].pieData.value / sumValue, 4) : 0;
    legendData.push({
      name: series[i].name,
      value: bfb
    });
...
const getHeight3D = (series, height) => {
  ...
  return (
    (series[0] && series[0].pieData.value === 0
      ? 0
      : (height * 25) / series[0].pieData.value) || 0
  );
};
